...
class TrainDataLoader(AbstractDataLoader):
    ...
    def get_group_uids(self, feature_column, group_id=0):
        return set(self.dataset.df[self.dataset.df[feature_column] == group_id][self.dataset.uid_field].unique())
    ...
    def _get_neg_sample(self):
        ...
        # fairness: add sensitive feature
...
class EvalDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, additional_dataset=None,
        ...
        self.eval_items_2darray = np.zeros((self.dataset.get_user_num(), self.dataset.get_item_num()), dtype=np.int32)

        self.eval_u = self.dataset.df[self.dataset.uid_field].unique()
        
        # Filter the dataframe for the user ids and select the relevant columns
        filtered_df = self.dataset.df[self.dataset.df[self.dataset.uid_field].isin(self.eval_u)][[self.dataset.uid_field] + self.dataset.feature_columns]

        # Drop duplicates to get unique sensitive attributes for each user
        unique_sensitive_attributes = filtered_df.drop_duplicates()[self.dataset.feature_columns]

        # Convert the dataframe to a list of lists
        self.eval_u_sens = unique_sensitive_attributes.values.tolist()
        ...
        self.eval_u_sens = torch.tensor(self.eval_u_sens).type(torch.LongTensor).to(self.device)
    ...
    def _next_batch_data(self):
        ...
        batch_users_sens = self.eval_u_sens[self.pr: self.pr + self.step]
        ...
        self.pr += self.step
        return [batch_users, batch_mask_matrix, batch_users_sens]
    ...
    def _get_eval_items_per_u(self, eval_users):
        ...
        all_items = np.concatenate(self.eval_items_per_u)  # Flatten all items in test_u2i
        all_user_indices = np.repeat(np.arange(self.dataset.get_user_num()), [len(items) for items in self.eval_items_per_u])
        self.eval_items_2darray[all_user_indices, all_items] = 1
    ...
    def get_eval_users_sens(self):
        return self.eval_u_sens.cpu()

    def get_eval_items_2darray(self):
        return self.eval_items_2darray
