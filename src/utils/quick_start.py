...
from utils.utils import init_seed, get_recommendation_model, get_fairness_model, dict2str, get_trainer
import platform
import os
...
def quick_start(recommendation_model, dataset, config_dict, save_model=True):
    ...
    for hyper_tuple in combinators:
        ...
        # to let at least one param be 0.05
        # no_005 = True

        # for param in hyper_tuple:
        #     if param == 0.05:
        #         no_005 = False
        #         break
        ...
        if config['fairness_model'] is None:
            ...
            # create afrl discriminators
            if config['fairness_model'] in ['AFRL', 'FairRec', 'DSPT1FairRec', 'DSPT1AFRL']:
                afrl_fair_disc_dict = {}
                for feat_idx in disc_data_reader.feature_info:
                    if disc_data_reader.feature_info[feat_idx].num_class == 2:
                        afrl_fair_disc_dict[feat_idx] = BinaryDiscriminator(disc_data_reader.feature_info[feat_idx], embedding_size = config['embedding_size'], config = config)
                    else:
                        afrl_fair_disc_dict[feat_idx] = MulticlassDiscriminator(disc_data_reader.feature_info[feat_idx], embedding_size = config['embedding_size'], config = config)
        ...
        if config['fairness_model'] in ['AFRL', 'FairRec', 'DSPT1FairRec', 'DSPT1AFRL']:
            trainer = get_trainer()(config, model, fair_disc_dict = fair_disc_dict, afrl_fair_disc_dict = afrl_fair_disc_dict)
        else:
            trainer = get_trainer()(config, model, fair_disc_dict = fair_disc_dict)
        # debug
        # model training
        if config['fairness_model'] in ['DAL', 'DALS', 'DALF', 'DALFS', 'DALFD', 'DALFDI', 'DALFDT', 'SAF_D', 'SAF_D_SC', 'MF', 'DSPT1', 'DSPT2', 'DSPT1WOS', 'DSPT1WOA', 'DSPT1FairRec', 'DSPT1FairGo', 'DSPT1AFRL']:
